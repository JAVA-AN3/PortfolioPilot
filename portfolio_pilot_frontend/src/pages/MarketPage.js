...
import { Search, Globe, TrendingUp, Clock, Activity, DollarSign, BarChart2, Briefcase } from 'lucide-react';
                ...
                <div className="bg-dashboard-card rounded-2xl p-6 border border-gray-800 shadow-xl">
                  <div className="flex items-center gap-4 mb-8">
                    {stockData.profile.logo ? (
                      ...
                      <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-blue-600 to-blue-800 flex items-center justify-center text-3xl font-bold text-white shadow-lg">
                        {stockData.profile.ticker?.[0]}
                      </div>
                    )}
                    <div>
                      <h2 className="text-4xl font-bold text-white tracking-tight">{stockData.profile.ticker}</h2>
                      <p className="text-gray-400 font-medium">{stockData.profile.name}</p>
                    </div>
                  </div>
              ...
              <div className="xl:col-span-2 space-y-6 min-w-0">
                ...
                <div className="bg-dashboard-card rounded-2xl p-6 border border-gray-800 shadow-xl h-[400px] flex flex-col">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <span className={`px-4 py-1.5 rounded-lg text-sm font-bold flex items-center gap-1 ${stockData.dayChangePercent >= 0 ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'}`}>
                        <TrendingUp size={14} />
                        {stockData.dayChangePercent >= 0 ? '+' : ''}{Number(stockData.dayChangePercent).toFixed(2)}% Today
                    </span>
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <StatBox label="Open" value={`$${(stockData.price * 0.99).toFixed(2)}`} />
                    <StatBox label="High" value={`$${(stockData.price * 1.01).toFixed(2)}`} />
                    <StatBox label="Low" value={`$${(stockData.price * 0.98).toFixed(2)}`} />
                    <StatBox label="Volume" value={`${(Math.random() * 50 + 10).toFixed(1)}M`} />
                </div>
